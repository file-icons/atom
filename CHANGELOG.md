...
----------------------
### Added
- **New icons:** Bosque (`.bsq`), Cubit (`cubit{,s}` directories), draw.io (`*.{drawio,dio}.{png,svg}`, `.drawio.*`), Fabric (`fabric.mod.json`), ftHTML (`.fthtml`), jscpd (`jscpd-report.{xml,html,json}`, `.jscpd.json`), LGTM (`lgtm.yml`), LINQPad (`.linq`), Q\# (`.qs`), xmake (`xmake.lua`)
