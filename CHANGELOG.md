...
----------------------
### Added
- **New icons:** Bosque (`.bsq`), Cubit (`cubit{,s}` directories), draw.io (`*.{drawio,dio}.{png,svg}`, `.drawio.*`), DVS (`.dvs` directories), Fabric (`fabric.mod.json`), ftHTML (`.fthtml`), jscpd (`jscpd-report.{xml,html,json}`, `.jscpd.json`), LGTM (`lgtm.yml`), Lighthouse (`{.,}lighthouserc.{yaml,yml,js,json}`), LINQPad (`.linq`), Q\# (`.qs`), xmake (`xmake.lua`)
