...
- **Support:** Generic code (`.cgi`, `.fcgi`), Jest (`jest.config.js`), Perl (`.al`), Shell-scripts (`.mksh`, `.pdksh`), TypeScript tests (`*.{spec|test}.{ts,tsx}`), Unit tests (`t/*.sh`, `t/*.t`, `.xspec`), Visual Studio (`.njsproj`)
