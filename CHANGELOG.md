...
- **Support:** 3D (`.step`, `.stp`, `.p21`), Bazel (`.bazel`), Config (`.sdl`, `.sdlang`), Config: Ruby (`.autotest`, `.cross_rubies`, `.gemtest`, `.hoerc`, `.kick`, `.simplecov`, `.yardopts*`), Bitcoin (`.bitcoin` directories, `bitcoin.conf`), Database (`.2da`), Grammar (`.cf`, `.lbnf`), LabVIEW (`.lvlib`), NumPY (`.npy`, `.npz`), NuGet (`.nupkg`, `.snupkg`, `.psmdcp`), Prettier (`.prettierrc.json5`), Python (`py.typed`), Ruby (`Gemfile_*`)
