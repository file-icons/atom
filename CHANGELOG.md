...
- **Support:** BibTeX (`.bbl`), Config (`.mcf`), Databases (`eign`, `term/tab.*`, `nterm/tab.{X,lpr}`), Fonts (`.dfont`, `.tfm`), Images (`.avif`, `.heif`, `.heifs`, `.hif`), GNU (`{A,L,}GPL*`), Lex (`.lx`), Makefile (`contrib.mak-*`), PostScript (`Fontmap`), Printer (`.joboptions`), Tag (`VERSION`)
