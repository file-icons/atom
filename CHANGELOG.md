...
----------------------
### Added
- **Support:** C (`share/misc/getopt{,s}{0..9}`), Database (`birthtoken`, `flowers`), FontForge (`.ff`), Roff (`{man,mdoc}.template`), Sieve (`.sieve`), TeX (`.texinfo`, `.txi`), Text (`share/misc/{operator,mail.{help,tildehelp}}`), Unit databases (`.units`, `units.lib`, `.units_history`)
