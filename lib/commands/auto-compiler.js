...
const LOCALLIB_PATH = "../lib/icons/icon-compiler.js";
...
class AutoCompiler{
	...
	recompile(){
		...
		const source = __dirname + "/../../config.cson";
		const target = __dirname + "/../icons/.icondb.js";
	...
	compileConfig(source, target){
		...
		if(this.currentJob)
			return source !== this.jobs.get(currentJob)
				? this.currentJob.then(_=> this.compileConfig(source, target))
				: this.currentJob;
		else
			return this.startTask(source, target).then(() => {
				for(const repo of atom.project.repositories)
					repo.projectAtRoot && repo.refreshStatus();
			})
	}
	...
	describeError(code, signal){
		...
		switch(code){
			...
			case ERROR_NO_PACKAGE_PATH: error = [
				...
				+ "**Failed to include:** `" + LOCALLIB_PATH + "`"
			]; break;
	...
	findTarget(){
		const fn = (source, target) => {
			const sourceFile = FileRegistry.get(source);
			const targetFile = FileRegistry.get(target);
