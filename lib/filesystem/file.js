...
const {lstat, statify, sampleFile} = require("../utils/fs.js");
...
class File extends Resource {
	...
	constructor(path, stats = null){
		super(path);
		
		stats = stats || lstat(path);
		if(stats){
			this.stats = stats;
	...
	onDidChangeOnDisk   (fn){ return this.emitter.on("did-change-on-disk",  fn)}
	...
	loadData(asap = false, limit = null){
		if(asap){
			...
			System.loadData(this.path, limit, this.symlink).then(result => {
				this.pendingData = false;
	...
	/**
	 * Load the file's absolute physical pathname.
	 *
	...
	loadRealPath(asap = false){
		if(asap){
			...
			if(to !== from && to !== this.realPath){
				this.realPath = to;
				setImmediate(_=> this.emit("did-change-realpath", {from, to}));
			}
		}
...
File.prototype.pendingStats = false;
File.prototype.pendingRealPath = false;
...
File.prototype.unreadable = false;
