...
class System{
	...
	loadData(path, limit = 80){
		return this.addRequest(path, "data", {limit});
	}
	...
	loadRealPath(path){
		return this.addRequest(path, "realpath");
	}
	...
	sendRequests(){
		...
		task.on("op:error", (type, path, error) => {
			this.errorLog.push([path, error, new Date()])
			const requestID = `${op[type]}:${path}`;
			const request = requests.get(requestID);
			if(request){
