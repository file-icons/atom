...
class Resource{
	...
	getIcon(){
		if(this.icon)
			...
			// TODO: Move this to a Strategy object
			const icon = IconRegistry.matchFile({
	...
	getIconClasses(){
		...
		if(this.symlink)
			iconClasses[0] = "icon-file-symlink-file";
		
		/**
		 * HACK: `iconClasses` is only stored because we have no control over when
		 * the service consumes icons, and IconDelegates need to know what classes
		 * to remove from their elements when icons change.
		 */
	...
	/**
	 * Modify the VCS status of this resource.
	 *
	 * @param {String} to - "modified" or "added"
...
Resource.prototype.vcsStatus = null;
