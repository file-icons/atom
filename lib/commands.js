...
class Commands {
	...
	init(){
		...
		this.add("recompile-config", () => this.recompileConfig());
	 ...
	 *
	 * TODO: Use a background task so this becomes non-blocking.
	 *
	 * @see {@link IconCompiler#recompile}
	 */
	recompileConfig(){
		const IconCompiler = require("./icons/icon-compiler.js");
		IconCompiler.recompile().then(() => {
			atom.notifications.addInfo("Config recompiled", {dismissable: true});
		});
	}
