...
class IconCompiler{
	...
	compileConfigData(source, cson = false){
		if(cson) source = require("coffeescript").eval(source);
	...
	compileList(input){
		...
		for(const key in input){
			...
			Object.assign(result.tags, defs.tags);
			result.push(... defs);
		}
	...
	compileEntry(key, value){
		...
		if(!Array.isArray(match))
			match = [[match, value.colour, {...value}]];
		...
		for(const i of match){
			...
			if(entryProps.tag)
				output.tags[entryProps.tag] = prevIcon || newIcon;
			...
			if(!entryProps.tag)         delete value.tag;
		}
	...
	indexLists(lists){
		...
		for(const icons of lists){
			...
			// QUESTION: Exactly what the hell was I doing when I wrote this crap?
			const {tags} = icons;
			for(const tag in tags){
				const icon = tags[tag];
				tags[tag] = icons.indexOf(icon);
			}
			indexedLists.push([icons, indexes, tags]);
		}
		return indexedLists;
	}
	...
	stringify(lists){
		let output =
			...
			"module.exports = [\n";
		for(const [iconList, indexList, tags] of lists)
			output += "["
				+ "[" + iconList.join(",\n") + "],\n"
				+ JSON.stringify(indexList) + ",\n"
				+ JSON.stringify(tags).replace(/"([^":]+)":/g, "$1:")
				+ "],\n";
