...
["manpage-icon",["dark-green","dark-green"],/\.(?:1(?:[bcmstx]|has|in)?|[24568]|3(?:avl|bsm|cfgadm|in|[cmx]|qt)?|7(?:d|fs|i|ipp|m|p)?|9[efps]?|chem|eqn|groff|man|mandoc|mdoc|me|mom|nr?|nroff|pic|roff?|tmac|tmac-u|tr|troff)$|(?:^|\.)(?:(?:troff|eqn)rc(?:-end)?)$/i,,false,/man|mandoc|(?:[gnt]|dit)?roff/i,/\.[gtn]?roff$/i,/^(?:[gtn]?roff|man(?:[-_\s]?page|doc)?|mdoc)$/i,/^('\\" [tre]+(?=\s|$)|\.TH[ \t]+(?:\S+)|\.so[ \t]+man(\w+)\/.+\.\1($|\s)|\.(EQ|PS|cstart)\n|('+\t?|\.\t)\\"\s)/m],
