 ...
 * @property {Object} taggedIcons    - Icons tagged with a retrievable identifier.
 * @class
 */
class IconTables{
	...
	constructor(){
		...
		this.taggedIcons    = {__proto__: null};
	...
	read(table){
		...
		for(const tag in tags)
			this.taggedIcons[tag] = icons[tags[tag]];
	...
	matchSignature(data){
		...
		if(/\0/.test(data)){
			const icon = this.taggedIcons.binary;
			cache.signature.set(data, icon);
			return icon;
		}
