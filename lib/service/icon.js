...
const {caseKludge, escapeRegExp, fuzzyRegExp, isNumeric, isRegExp, isString} = require("../utils/general.js");
...
class Icon{
	...
	constructor(){
		...
		if(arguments.length === 1){
			...
			if(null != match[2])    this.matchPath   = !!match[2];
		}
		...
		// Parsing from CSON config
			...
			// Icon is associated with a language
			if(scope){
	...
	/**
	 * Restore a list of Icons from their JSON representation.
	 *
	 * @param {Array} input
	 * @return {Array}
	 */
	...
	canMerge(input){
		...
		if(!!this.matchPath !== !!input.matchPath) return false;
	 ...
	 * @private
	 */
	static parseMatchPattern(input, fixSeparators = false){
		if(isRegExp(input))
			return input;
